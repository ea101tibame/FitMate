...
//DAO2 測試 同時新增 時段跟明細
public class LessonDAO2 implements LessonDAO_interface {
	...
	private static final String INSERT_LessonTime_STMT="INSERT INTO LESSON_TIME VALUES ('LT'||LPAD(to_char(LTIME_NO_seq.NEXTVAL), 3, '0'),to_date(?, 'yyyy-mm-dd'),?)";
...
//			LessonDAO2 dao = new LessonDAO2();//新增完還未COMMIT 拿不到
...
*/	
			...
			/*回到原版*/
			pstmt.setString(1,"2020-07-01");//上課程日期
			pstmt.setInt(2, 1);//時段
			pstmt.executeUpdate();
			System.out.println("新增一筆時間到Time");
			...
			pstmt.setString(1,"2020-07-03");//上課程日期
			pstmt.setInt(2, 0);//時段			
			pstmt.executeUpdate();
			...
			ResultSet rs = pstmt.getGeneratedKeys();//拿出pstmt = con.prepareStatement(INSERT_ORDER, cols);剛剛新增的訂單編號
				if (rs.next()) {
					...
					System.out.println("自增主鍵值 = " + next_ltime_no + "(剛新增成功的訂單編號)");
				} else {
...
//		再拿一次他的COL 給下面用
	...
	public void insert(LessonVO lessonVO) {
		...
		try {
			...
			//拿出pstmt = con.prepareStatement(INSERT_LESSNO, cols);剛剛新增的訂單編號
			if (rs.next()) {
				next_lessNo = rs.getString(1);
				System.out.println("自增主鍵值 = " + next_lessNo + "(剛新增成功的訂單編號)");
			} else {
				System.out.println("未取得自增主鍵值");
			}
			rs.close();
