	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Update".equals(action)) {
			...
			try {
				...
				/* 查詢完準備轉交 */
				req.setAttribute("activity_orderVO", activity_orderVO);
				String url = "/front-end/activity_order/update_activityorder_input.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
			} catch (Exception e) {
				errorMsgs.add("無法取得要修改的資料:" + e.getMessage());
				RequestDispatcher failureView = req.getRequestDispatcher("/front-end/activity_order/listAllActivityOrder.jsp");
				failureView.forward(req, res);
			}
		}
		/* 修改 */
		if ("update".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("activity_orderVO", activity_orderVO);
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/activity_order/update_activityorder_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
				/* 開始修改 */
