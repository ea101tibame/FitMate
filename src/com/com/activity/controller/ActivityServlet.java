	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/activity/selectActivity_page.jsp");
		...
		if ("getOne_For_Update".equals(action)) {
			...
			try {
				...
				/* 查詢完準備轉交 */
				req.setAttribute("activityVO", activityVO);
				String url = "/activity/update_activity_input.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
			} catch (Exception e) {
		...
		if ("update".equals(action)) {
			...
			try {
				...
				String coanoReg = "^[(\u4e00-\u9fa5)(a-zA-Z0-9_)]{2,10}$";
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("activityVO", activityVO);
					RequestDispatcher failureView = req.getRequestDispatcher("/activity/update_activity_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
				...
				activityVO = activitySvc.updateActivity(actno, actname, actloc, actdate, actss, actstart, actend,
						acttype, actprice, actmin, actmax, actcur, actdesc, actsta, actpic, stuno, coano);
				/* 修改完準備轉交 */
				req.setAttribute("activityVO", activityVO);
				String url = "/activity/listOneActivity.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/emp/update_activity_input.jsp");
				failureView.forward(req, res);
			}
		}
		...
		if ("insert".equals(action)) {
			...
			try {
				...
				activityVO = activitySvc.addActivity(actname, actloc, actdate, actss, actstart, actend, acttype,
						actprice, actmin, actmax, actcur, actdesc, actsta, actpic, stuno, coano);
				req.setAttribute("activityVO", activityVO);
				/* 新增完準備轉交 */
